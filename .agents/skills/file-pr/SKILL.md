...
- **On a Mac with Xcode:** use the iOS Simulator, which runs the native app. For an existing PR, use [simulator-check](../simulator-check/SKILL.md). Otherwise start it as the [README](../../../README.md) describes, or drive a Maestro flow with `E2E_FLOW=<flow> npm run e2e:ios` (see [verification](../../../docs/verification.md)). Capture with `xcrun simctl io booted screenshot <name>.png` and `xcrun simctl io booted recordVideo --codec=h264 <name>.mp4`, stopping the recording with Ctrl-C. GitHub rejects videos over 10 MB, so keep recordings short or re-encode them with ffmpeg.
