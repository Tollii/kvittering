...
2. Create `convex/convex.config.ts`:
   ...
   export default app;
   ```
3. Store Stripe keys in Convex env (follow backend operations): `STRIPE_SECRET_KEY` (sk_test_… / sk_live_…) and `STRIPE_WEBHOOK_SECRET` (whsec_…).
...
- Stripe keys live in Convex env (follow backend operations): STRIPE_SECRET_KEY and STRIPE_WEBHOOK_SECRET.
