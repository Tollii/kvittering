import { z } from 'zod';
import { emptyLine, lineKinds, osloDate, validateReceipt, type ReceiptData } from './receipt';
...
export function prepareExtraction(
	...
	if (purchaseDate && /^--\d{2}-\d{2}$/.test(purchaseDate)) {
		const candidate = osloDate(referenceTime).slice(0, 4) + purchaseDate.slice(1);
		const parsed = new Date(candidate);
		if (Number.isFinite(parsed.getTime()) && parsed.toISOString().slice(0, 10) === candidate) {
			purchaseDate = candidate;
		} else {
			purchaseDate = null;
			issues.push('Datoen er ikke gyldig i inneværende år. Kontroller kjøpsdatoen.');
		}
	}
	const data: ReceiptData = {
		...
		purchaseDate,
		issues,
		lines: extraction.lines.map(({ sourceImages, overlapUncertain, ...line }) => {
			const sources = [...new Set(sourceImages)].sort((a, b) => a - b);
			const validSources =
				sources.length > 0 &&
				sources.every((image) => Number.isInteger(image) && image >= 1 && image <= imageCount);
			return {
				...emptyLine(line.id),
				...line,
				...
				issues: [
					...new Set([
						...
						...(overlapUncertain
							...
							: []),
						...(!validSources ? ['Kunne ikke knytte linjen sikkert til et bilde.'] : [])
					])
				],
