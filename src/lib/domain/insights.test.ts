...
it('handles year boundaries, leap days and completed months', () => {
	expect(comparisonInsights([], '2026-01', false, '2026-01-17').previousEnd).toBe('2025-12-17');
	expect(comparisonInsights([], '2024-03', false, '2024-03-31').previousEnd).toBe('2024-02-29');
	const result = comparisonInsights([receipt('2026-08-31')], '2026-08', false, '2026-09-17');
	expect(result.current.products).toBe(1000);
	expect(result.partial).toBe(false);
});
function contribution(date: string, ore: number, size: number | null): Contribution {
	return {
		receipt: receipt(date),
		line: { ...emptyLine(), quantity: 1, packageSize: size, packageUnit: size ? 'g' : null },
		amountOre: ore
	};
}
...
it('distinguishes automatic links from user confirmation and excludes duplicate coverage', () => {
	const line = {
		...emptyLine(),
		productId: 'product' as NonNullable<ReturnType<typeof emptyLine>['productId']>
	};
	expect(matchLabel(line)).toBe('Automatisk koblet');
	expect(matchLabel({ ...line, productMatchManual: true })).toBe('Bekreftet av deg');
	const item = receipt('2026-09-01');
	expect(receiptCoverage([item, { ...item, excluded: true }]).unlinkedCount).toBe(1);
});
...
it('aggregates calendar dates with discounts, excludes pant, and applies receipt filters', () => {
	...
	const second = { ...receipt('2026-09-07', 1000), status: 'needs_review' } as Receipt;
	const excluded = { ...receipt('2026-09-07', 9000), excluded: true };
	const foreign = { ...receipt('2026-09-07'), data: { ...data, currency: 'EUR' } };
...
it('keeps leap days, empty dates and negative totals without colouring refunds as spending', () => {
	const calendar = spendingCalendar([receipt('2024-02-29', -1000)], 2024, false, '2025-01-01');
	expect(calendar).toHaveLength(366);
	expect(calendar.find((d) => d.date === '2024-02-29')).toMatchObject({
		amountOre: -1000,
		level: 0
	});
	expect(calendar[0]).toMatchObject({ amountOre: 0, level: 0, contributions: [] });
});
