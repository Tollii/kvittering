...
export const spendingTotalsValidator = v.object({
  products: oreValidator,
  paid: oreValidator,
  discounts: oreValidator,
  deposits: oreValidator,
  returns: oreValidator,
  receipts: v.number(),
  provisional: v.number(),
  unconverted: v.number(),
  unknownTotals: v.number(),
  unknownAmounts: v.number(),
  comparisonProducts: oreValidator,
...
export function addSpendingTotals(
  ...
  return {
    products: Ore.add(left.products, Ore.scale(right.products, sign)),
    paid: Ore.add(left.paid, Ore.scale(right.paid, sign)),
    discounts: Ore.add(left.discounts, Ore.scale(right.discounts, sign)),
    deposits: Ore.add(left.deposits, Ore.scale(right.deposits, sign)),
    returns: Ore.add(left.returns, Ore.scale(right.returns, sign)),
    ...
    comparisonProducts: Ore.add(
      left.comparisonProducts,
      Ore.scale(right.comparisonProducts, sign),
    ),
    comparisonReceipts:
...
export function receiptSpendingTotals(receipt: Receipt): SpendingTotals {
  ...
  const insight = monthlyInsights([receipt], CalendarDate.month(date));
  ...
  return {
    ...
    comparisonProducts: comparable ? insight.products : Ore.zero,
