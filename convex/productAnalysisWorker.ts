...
export function familyQuestion(families: { name: string }[]) {
  ...
  families.forEach((family, index) => {
    options[`family_${index}`] =
      `Same product family as candidates[${index}]: ${family.name}`;
  });
  return choice(familyRules, options);
}
...
export const analyze = internalAction({
  ...
  handler: async (ctx, args): Promise<ProductAnalysisResult[]> => {
    ...
    for (const line of receipt.data.lines.filter(
      ...
      let context = await ctx.runQuery(internal.productAnalysis.prepare, {
        ...args,
        lineId: line.id,
      });
      if (!context) return [];
      if (!context.profile) {
        ...
        const countIndex = /^count_(\d+)$/.exec(response.answers.count.choice);
        const unitsPerPackage = countIndex
          ? (candidates.counts[Number(countIndex[1])] ?? null)
          : null;
        const selectedMeasure = /^(total|each)_(\d+)$/.exec(
          response.answers.measure.choice,
        );
        const value = selectedMeasure
          ? candidates.measures[Number(selectedMeasure[2])]
          : null;
        const multiplier =
          selectedMeasure?.[1] === "each" ? unitsPerPackage : 1;
        const measurePerPackage =
          value && multiplier
            ? { ...value, amount: value.amount * multiplier }
            : null;
        const familyIndex = /^family_(\d+)$/.exec(
          response.answers.family.choice,
        );
        const family = familyIndex
          ? (context.families[Number(familyIndex[1])]?._id ?? null)
          : response.answers.family.choice === "new"
            ? ("new" as const)
            : null;
        await ctx.runMutation(internal.productAnalysis.saveProfile, {
          ...args,
          lineId: line.id,
        ...
        context = await ctx.runQuery(internal.productAnalysis.prepare, {
          ...args,
          lineId: line.id,
        });
      }
      ...
      const family = context.families.find(
        (family) => family._id === context.profile!.familyId,
      );
      prepared.push({
        line,
