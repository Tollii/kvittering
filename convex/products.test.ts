...
async function setup() {
	...
	const user = t.withIdentity({ subject: 'member', issuer: 'test', name: 'Member' });
	...
	async function receipt() {
		...
		await t.run((ctx) =>
			ctx.db.patch('receipts', id, { data: batteryFixture(), status: 'needs_review' })
		);
		return id;
	}
...
it('saves a product correction, reuses the retailer mapping and preserves the original text', async () => {
	...
	const repeated = { ...line, receiptName: '  battery   remix ', productId: null };
	...
	expect(
		(await t.query(internal.products.prepare, { id, retailer: 'Another', line: repeated })).saved
	).toBe(false);
	...
	expect((await user.query(api.receipts.detail, { id })).receipt.data!.lines[0].productId).toBe(
		line.productId
	);
});
it('keeps an explicit separation for future receipts and refuses foreign household products', async () => {
	...
	await expect(
		outsider.query(api.products.search, { receiptId: id, retailer: 'Eksempelbutikk', search: '' })
	).rejects.toThrow('ikke tilgjengelig');
});
it('finishes uncertain processing and atomically reuses new product mappings on retries', async () => {
	...
	const one = (await user.query(api.receipts.detail, { id: first })).receipt.data!.lines[0];
	const two = (await user.query(api.receipts.detail, { id: second })).receipt.data!.lines[0];
	expect(two.productId).toBe(one.productId);
	expect(
		await t.run((ctx) =>
			ctx.db
				.query('products')
				.withIndex('by_householdId_and_retailer', (q) => q.eq('householdId', householdId))
				.take(10)
		)
	).toHaveLength(1);
	const third = await receipt();
	await t.run((ctx) => ctx.db.patch('receipts', third, { status: 'processing', generation: 1 }));
	const data = batteryFixture();
...
it('continues with separate items when the matching provider fails', async () => {
	...
	try {
		const matches = await t.action(internal.productMatching.match, { id, data: batteryFixture() });
		expect(matches).toEqual([{ lineId: 'battery', kind: 'uncertain', productId: null }]);
...
it('accepts a clear Jev candidate and refuses low confidence without creating a link', async () => {
					...
					JSON.stringify({
						...
						answers: {
							item_0: {
								...
								choice: 'candidate_0',
								confidence,
								probabilities: { candidate_0: 0.95, new_product: 0.01, uncertain: 0.04 }
							}
						},
	...
	try {
		expect(await t.action(internal.productMatching.match, { id, data: batteryFixture() })).toEqual([
			{ lineId: 'battery', kind: 'match', productId }
		]);
		confidence = 0.4;
		expect(await t.action(internal.productMatching.match, { id, data: batteryFixture() })).toEqual([
