...
import { productEvidence, matchingInstructions } from '../src/lib/domain/product-matching';
	...
	handler: async (ctx, args): Promise<Infer<typeof productDecision>[]> => {
		...
		for (let offset = 0; offset < lines.length; offset += 12) {
			...
			const prepared = await Promise.all(
				batch.map((line) =>
					ctx.runQuery(internal.products.prepare, { id: args.id, retailer: args.data.store!, line })
				)
			);
			const unresolved = batch
				.map((line, index) => ({ line, index, candidates: prepared[index].candidates }))
				.filter(({ index }) => {
			...
			try {
				...
				unresolved.forEach((item, index) => {
					...
					const candidate = item.candidates.find((_, i) => answer.choice === `candidate_${i}`);
					if (candidate)
					...
					else if (answer.choice === 'new_product') decisions[offset + item.index].kind = 'new';
				});
			} catch {
