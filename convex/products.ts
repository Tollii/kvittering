...
import { lineValidator, type ReceiptData, type ReceiptLine } from '../src/lib/domain/receipt';
import {
	matchingKey,
	compatibleProduct,
	productEvidence,
		...
		tokens.map((token) =>
			ctx.db
				...
				.withSearchIndex('search_name', (q) =>
					q.search('name', token).eq('householdId', householdId).eq('retailer', retailer)
				)
				.take(8)
		)
	);
...
export const search = query({
	args: { receiptId: v.id('receipts'), retailer: v.string(), search: v.string() },
	...
	handler: async (ctx, args) => {
		...
		if (args.search.trim())
			return (await candidates(ctx, member.householdId, retailer, args.search.slice(0, 200))).slice(
				0,
				20
			);
		return ctx.db
...
export const prepare = internalQuery({
	...
	handler: async (ctx, args) => {
		...
		const retailer = matchingKey(args.retailer);
		const mapping = await findMapping(ctx, receipt.householdId, retailer, args.line);
		if (mapping) {
			const product = mapping.productId ? await ctx.db.get('products', mapping.productId) : null;
			if (
				!mapping.productId ||
				(product && compatibleProduct(args.line, product, mapping.confirmedBy !== null))
			)
...
export async function linkProduct(
	...
	if (id && (!product || product.householdId !== householdId || product.retailer !== retailer))
...
export async function correctProducts(
	...
	editor: string,
	data: ReceiptData,
	changes: { lineId: string; productId: Id<'products'> | null; createNew: boolean }[]
) {
	...
	for (const change of changes) {
		const line = data.lines.find((l) => l.id === change.lineId && l.kind === 'product');
