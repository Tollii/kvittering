...
export const processReceipt = workflow
	...
	.handler(async (step, args): Promise<null> => {
		try {
			const storageIds = await step.runMutation(internal.processing.begin, args);
			if (!storageIds) return null;
			const extraction: { data: ReceiptData; provider: string } = await step.runAction(
			...
			await step.runMutation(internal.processing.fail, {
				...args,
				error: error instanceof Error ? error.message : 'Behandlingen mislyktes.'
			});
		}
		return null;
	});
export const begin = internalMutation({
	...
	handler: async (ctx, args) => {
		...
		if (!receipt || receipt.generation !== args.generation || receipt.status !== 'uploaded')
			return null;
		...
		const images = await ctx.db
			...
			.take(8);
		return images.sort((a, b) => a.position - b.position).map((image) => image.storageId);
	}
});
...
export const finish = internalMutation({
	...
	handler: async (ctx, args) => {
		const receipt = await ctx.db.get('receipts', args.id);
		if (!receipt || receipt.generation !== args.generation || receipt.status !== 'processing')
			return null;
		...
		if (!duplicateOf && args.data.store && args.data.purchaseDate && args.data.totalOre !== null) {
			const others = await ctx.db
				.query('receipts')
				.withIndex('by_householdId', (q) => q.eq('householdId', receipt.householdId))
				.order('desc')
				.take(250);
			const duplicate = others.find(
				(other) =>
					...
					other.data?.store &&
					normalizeAlias(other.data.store) === normalizeAlias(args.data.store!) &&
					other.data.purchaseDate === args.data.purchaseDate &&
					other.data.totalOre === args.data.totalOre &&
					((args.data.receiptNumber && other.data.receiptNumber === args.data.receiptNumber) ||
						(args.data.purchaseTime && other.data.purchaseTime === args.data.purchaseTime))
			);
