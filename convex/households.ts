...
export const join = mutation({
	...
	handler: async (ctx, args) => {
		...
		const household = await ctx.db
			.query('households')
			.withIndex('by_invitation', (q) => q.eq('invitation', args.invitation.trim()))
			.unique();
		...
		const members = await ctx.db
			...
			.take(2);
		if (members.length >= 2) throw new Error('Husstanden har allerede to medlemmer.');
		await ctx.db.insert('members', {
...
export const rotateInvitation = mutation({
	...
	handler: async (ctx, args) => {
		...
		if (!/^[a-f0-9]{32}$/.test(args.invitation)) throw new Error('Ugyldig invitasjon.');
		await ctx.db.patch('households', member.householdId, { invitation: args.invitation });
		return null;
	}
});
